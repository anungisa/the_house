/**
 * Shared identity resolution for the Participant Registry HTTP read surface.
 *
 * The read endpoints carry identity EXCLUSIVELY in the shared `x-house-*` trusted-header contract
 * and NEVER source tenant/actor from the query string or path. This module centralizes that
 * contract so the participant list/detail and organization-participant list adapters resolve
 ...
 */
export function resolveParticipantAuth(
...
/** Require a non-blank tenant identity for any participant read. Fails closed (401) otherwise. */
export function requireTenant(auth: AuthContext): string {
  if (auth.tenantId.trim() === '') {
    throw new UnauthenticatedError('Participant registry reads require a tenant identity.');
  }
