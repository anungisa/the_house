/**
 * Shared identity resolution for the Facility Registry HTTP read surface.
 *
 ...
 * contract so the facility list/detail and organization-facilities list adapters resolve identity
 * identically.
 *
 ...
 */
export function resolveFacilityAuth(
...
/** Require a non-blank tenant identity for any facility read. Fails closed (401) otherwise. */
export function requireTenant(auth: AuthContext): string {
  if (auth.tenantId.trim() === '') {
    throw new UnauthenticatedError('Facility registry reads require a tenant identity.');
  }
