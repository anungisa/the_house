...
/**
 * Synthesize a demo-resolver identity body from the trusted-header contract. The decision
 * body carries no identity, so demo identity is sourced from `x-house-*` headers (trusted
 * without verification in demo mode), mirroring the evidence adapter.
 */
...
function requireTenant(auth: AuthContext): string {
  if (auth.tenantId.trim() === '') {
    throw new UnauthenticatedError('Workflow decisions require a tenant identity.');
  }
  return auth.tenantId;
}

function requireActorUserId(auth: AuthContext): string {
  if (auth.actor.userId.trim() === '') {
    throw new UnauthenticatedError('Workflow decisions require an actor identity.');
  }
