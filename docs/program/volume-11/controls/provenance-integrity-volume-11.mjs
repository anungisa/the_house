// Control: Volume 11 provenance-integrity enforcement (deterministic, NON-AUTHORITATIVE).
//
// Proves, from the source-controlled Volume 11 corpus alone, that Package 1's
// recorded provenance is coherent and role-correct. It evaluates the authoritative
// provenance record(s) - the REG-1105 approval(s) carrying a
...
// recorded only by the post-merge provenance amendment (V11-B-1), because it must
...
function approvals(ctx) {
  return ctx.registers?.['REG-1105']?.doc?.records ?? [];
}
...
function gateApproval(ctx) {
  return approvals(ctx).find((a) => /^GATE-V11-G[0-9]$/.test(a.artifact_id ?? '') && a.approval_state === 'ratified' && a.gate_disposition);
}
...
export function evaluateRecord(ctx, record) {
  ...
  const freezeArtifact = record.requires_freeze_artifact ?? rc.package ?? 'PACKAGE-11-1';
  ...
  add(11, 'Documentary effectiveness not treated as implementation or operational effectiveness', documentaryNotImplementation,
    `implementation_effective_date=${impl || '(missing)'}`);
  const anyAuthorizes = approvals(ctx).some((a) => a.authorizes_implementation !== false || a.authorizes_operations !== false);
  add(12, 'No record authorizes implementation or operations', !anyAuthorizes);
...
export function run(ctx) {
  ...
  if (recs.length === 0) {
    findings.push(makeFinding(Severity.INFO, 'PROVENANCE_AMENDMENT_PENDING', 'No REG-1105 approval yet carries a provenance_role_classification block; the post-merge provenance amendment (V11-B-1) is pending', 'REG-1105'));
    return findings;
  }
...
export function generate(ctx = loadContext()) {
  ...
  const chronology = {
    package: primary?.package ?? 'PACKAGE-11-1',
    inherited_release_tag: roles.inheritedTag ?? 'central-registration-volume-10-v1.0.0',
    sequence: [
  ...
  const roleClassification = {
    ...
    classification: {
      ...
      [roles.amendmentAuthoring ?? 'unknown']: 'V11_B_PROVENANCE_AMENDMENT_AUTHORING',
      [roles.amendmentMerge ?? 'unknown']: 'V11_B_PROVENANCE_AMENDMENT_MERGE'
    },
  ...
  const packageIndex = {
    ...
    note: 'Package-indexed projection of every authoritative Package 1-N provenance-role-classification record. Non-authoritative; confers no ratification and authorizes no implementation or operations.'
  };
  ...
  const md = `# Volume 11 Package 1 Provenance-Integrity Report (NON-AUTHORITATIVE)

Generated: ${now}

> This report is a generated, non-authoritative projection of the source-controlled
> Volume 11 corpus. It proves provenance coherence deterministically; it confers no
