...
const CONTROLLED_REGISTERS = ['REG-1101', 'REG-1102', 'REG-1103', 'REG-1104'];
...
// Implementation-authorization guard (fail closed): no Volume 11 record in any
  ...
  for (const regId of Object.keys(REGISTER_SCHEMAS)) {
    for (const r of records(ctx, regId)) {
      if (r.authorizes_implementation === true) {
        findings.push(makeFinding(Severity.ERROR, 'IMPLEMENTATION_UNAUTHORIZED', `${r.id}: authorizes_implementation must be false (Volume 11 operational governance foundation cannot authorize implementation, provisioning, execution, procurement, or release)`, r.id));
      }
    }
  }
}

// Operations-authorization guard (fail closed): no Volume 11 record in any
  ...
  for (const regId of Object.keys(REGISTER_SCHEMAS)) {
    for (const r of records(ctx, regId)) {
      if (r.authorizes_operations === true) {
        findings.push(makeFinding(Severity.ERROR, 'OPERATIONS_UNAUTHORIZED', `${r.id}: authorizes_operations must be false (Volume 11 operational governance foundation cannot authorize operations, migration, backup, restore, recovery, training, provider integration, or any operational exercise)`, r.id));
      }
    }
  }
}

// Operational-posture guard (fail closed): every controlled record must carry an
// implementation_status of NOT_IMPLEMENTED_OR_NOT_PROVEN, an operational_status
// of NOT_OPERATIONAL_OR_NOT_PROVEN, and an execution_status of NOT_EXECUTED.
  ...
  for (const regId of CONTROLLED_REGISTERS) {
    for (const r of records(ctx, regId)) {
      ...
      if (r.operational_status !== 'NOT_OPERATIONAL_OR_NOT_PROVEN') {
        findings.push(makeFinding(Severity.ERROR, 'OPERATIONAL_STATUS_INVALID', `${r.id}: operational_status must be NOT_OPERATIONAL_OR_NOT_PROVEN (an operational definition is not an operating service)`, r.id));
      }
      if (r.execution_status !== 'NOT_EXECUTED') {
        findings.push(makeFinding(Severity.ERROR, 'EXECUTION_STATUS_INVALID', `${r.id}: execution_status must be NOT_EXECUTED (no operational, migration, backup, recovery, training, or provider exercise has been executed)`, r.id));
      }
    }
  }
}
...
function validateFutureGates(ctx, findings) {
  ...
  for (const r of records(ctx, 'REG-1104')) {
