// Control: generate a NON-AUTHORITATIVE Volume 4 governance control report.
//
// The report is a projection of the source-controlled Volume 4 corpus. It is
...
import { run as runStructural } from './validate-volume-4.mjs';
...
export function buildReport(ctx, grouped) {
  ...
  const approvals = ctx.registers['REG-405']?.doc?.records ?? [];
  const conditions = approvals
    .flatMap((a) => (a.conditions ?? []).map((c) => `- ${a.id} (${a.artifact_id}): ${c}`))
    .join('\n');

  return `# Volume 4 Governance Control Report (NON-AUTHORITATIVE)
...
- Info: ${totals.info}
- Overall: ${totals.errors > 0 ? 'FAIL (architecture-foundation integrity errors present)' : 'PASS (no integrity errors)'}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  console.log(`Volume 4 control report written to ${outPath}`);
}
