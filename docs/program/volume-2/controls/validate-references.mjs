// Control: cross-reference (referential) integrity for the Volume 2 corpus.
//
...
// Level prefix of a requirement/outcome id (e.g. "CAP-V2-001" -> "CAP").
function levelOf(id) {
...
export function run(ctx) {
  ...
  const outcomeIds = idSet(ctx, 'REG-201');
  const stakeholderIds = idSet(ctx, 'REG-202');
  const requirementIds = idSet(ctx, 'REG-203');
  const decisionIds = idSet(ctx, 'REG-204');
  const approvalIds = idSet(ctx, 'REG-205');
  ...
  const corpus = records(ctx, 'REG-200');
  ...
  for (const rec of corpus) {
    ...
    if (!chapter) {
      findings.push(
        makeFinding(Severity.ERROR, 'CORPUS_MISSING_CHAPTER', `REG-200 indexes ${rec.id} but no chapter file exists`, rec.id)
      );
      continue;
    }
    if (rec.path && !existsSync(join(VOLUME_DIR, '..', '..', '..', rec.path))) {
      findings.push(
        makeFinding(Severity.ERROR, 'CORPUS_BAD_PATH', `REG-200 path for ${rec.id} does not exist: ${rec.path}`, rec.id)
      );
    }
  ...
  for (const ch of ctx.chapters) {
    if (ch.status === 'RATIFIED' && !indexed.has(ch.fileId)) {
      findings.push(
        makeFinding(Severity.ERROR, 'UNINDEXED_RATIFIED', `${ch.fileId}: RATIFIED chapter is not indexed in REG-200`, ch.fileId)
      );
    }
  }

  // Outcome cross-references.
  for (const out of records(ctx, 'REG-201')) {
  ...
  for (const req of records(ctx, 'REG-203')) {
  ...
  for (const dec of records(ctx, 'REG-204')) {
    for (const ref of dec.evidence_refs ?? []) {
      const isGate = /^GATE-V2-G[0-9]$/.test(ref);
  ...
  // Approval references.
  for (const app of records(ctx, 'REG-205')) {
    const art = app.artifact_id;
    const isPackage = /^PACKAGE-2-[0-9]$/.test(art);
    const isVolume = /^VOLUME-2$/.test(art);
    const isGate = /^GATE-V2-G[0-9]$/.test(art);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  await runStandalone('Volume 2 cross-reference integrity', run);
}
