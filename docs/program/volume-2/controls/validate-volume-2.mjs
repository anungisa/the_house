// Control: structural and schema conformance for the Volume 2 corpus.
//
...
// integrity, and the requirement-authorization guard (REG-203) that keeps
// Volume 2 product definition from authorizing implementation on its own
// (fail closed).
...
function validateStakeholderIntegrity(ctx, findings) {
  const stk = ctx.registers['REG-202'];
  if (!stk) return;
...
// Requirement-authorization guard: a Volume 2 requirement may never authorize
...
function validateRequirementAuthorization(ctx, findings) {
  const reg = ctx.registers['REG-203'];
  if (!reg) return;
    ...
    if (r.authorizes_implementation === true) {
      findings.push(
        makeFinding(
          ...
          `${r.id}: authorizes_implementation must be false (Volume 2 product definition cannot authorize construction)`,
          r.id
        )
      );
    }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  await runStandalone('Volume 2 structural and schema conformance', run);
}
