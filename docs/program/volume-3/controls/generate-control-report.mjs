// Control: generate a NON-AUTHORITATIVE Volume 3 governance control report.
//
// The report is a projection of the source-controlled Volume 3 corpus. It is
...
import { run as runStructural } from './validate-volume-3.mjs';
...
export function buildReport(ctx, grouped) {
  ...
  const approvals = ctx.registers['REG-305']?.doc?.records ?? [];
  const conditions = approvals
    .flatMap((a) => (a.conditions ?? []).map((c) => `- ${a.id} (${a.artifact_id}): ${c}`))
    .join('\n');

  return `# Volume 3 Governance Control Report (NON-AUTHORITATIVE)
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  console.log(`Volume 3 control report written to ${outPath}`);
}
