// Control: deterministic Volume 3 traceability projection (`npm run governance:trace:v3`).
//
...
// validation backlog for the source-controlled Volume 3 corpus.
//
...
// the authoritative record. Volume 0, Volume 1, and Volume 2 remain frozen.
...
// Ordered list of every requirement-chain record: OUT (REG-301) then REG-303.
function chainRecords(ctx) {
  const out = records(ctx, 'REG-301').map((r) => ({
    id: r.id,
    level: 'OUT',
  ...
  const reqs = records(ctx, 'REG-303').map((r) => ({
    id: r.id,
    level: r.level,
    product: r.product ?? null,
...
function makeResolver(ctx) {
  const known = new Set();
  for (const regId of ['REG-301', 'REG-302', 'REG-303', 'REG-304', 'REG-305']) {
...
function buildAuthorityBoundary(ctx) {
  ...
  const authorizeImplementation = records(ctx, 'REG-303')
...
function buildMarkdown(all) {
  ...
  return `# Volume 3 Traceability Report (NON-AUTHORITATIVE)
...
export function generate(ctx = loadContext()) {
  ...
  writeFileSync(join(outDir, 'volume-3-traceability-report.md'), markdown, 'utf8');
  return { outDir, all };
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const ac = all['acceptance-coverage'];
  const vb = all['validation-backlog'];
  console.log('=== Volume 3 traceability projection ===');
