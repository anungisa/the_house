// Control: freeze and amendment integrity for frozen Volume 1 packages.
//
// A frozen package must have a closure record, a closure decision, an approval,
// a recorded base commit, a stated amendment process, and a frozen-artifact
// version snapshot. Once frozen, any change to an artifact's ratified version
// requires a matching amendment decision.
...
export function run(ctx) {
  const findings = [];
  const approvals = records(ctx, 'REG-108');
  const decisions = records(ctx, 'REG-107');
  const chapterIds = new Set(ctx.chapters.map((c) => c.fileId));
  const corpusVersion = new Map(records(ctx, 'REG-100').map((r) => [r.id, r.version]));
  ...
  for (const app of packageApprovals) {
    if (!app.closure_record || !chapterIds.has(app.closure_record)) {
      ...
      if (!closureDecision) {
        findings.push(
          makeFinding(Severity.WARNING, 'FREEZE_NO_DECISION', `${app.id}: no closure decision in REG-107 references ${app.closure_record}`, app.id)
        );
      }
    }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  await runStandalone('Volume 1 freeze and amendment integrity', run);
}
