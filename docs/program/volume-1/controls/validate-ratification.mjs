// Control: ratification integrity for the Volume 1 corpus.
//
// A chapter/annex may claim RATIFIED only when it has an owner and approver, a
// covering ratified approval in REG-108 (direct artifact_id or package scope), an
...
export function run(ctx) {
  ...
  for (const ch of ctx.chapters) {
    ...
    if (!cover) {
      findings.push(
        makeFinding(Severity.ERROR, 'FALSE_RATIFICATION', `${ch.fileId}: RATIFIED but no covering ratified approval in REG-108`, ch.fileId)
      );
    }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  await runStandalone('Volume 1 ratification integrity', run);
}
