// Control: Volume 12 provenance-integrity enforcement (deterministic, NON-AUTHORITATIVE).
//
// Proves, from the source-controlled Volume 12 corpus alone, that Package 1's
// recorded provenance is coherent and role-correct. It evaluates the authoritative
// provenance record(s) - the REG-1205 approval(s) carrying a
...
// recorded only by the post-merge provenance amendment (V12-B-1), because it must
...
function approvals(ctx) {
  return ctx.registers?.['REG-1205']?.doc?.records ?? [];
}
...
function gateApproval(ctx) {
  return approvals(ctx).find((a) => /^GATE-V12-G[0-9]$/.test(a.artifact_id ?? '') && a.approval_state === 'ratified' && a.gate_disposition);
}
...
export function evaluateRecord(ctx, record) {
  ...
  const freezeArtifact = record.requires_freeze_artifact ?? rc.package ?? 'PACKAGE-12-1';
...
export function run(ctx) {
  ...
  if (recs.length === 0) {
    findings.push(makeFinding(Severity.INFO, 'PROVENANCE_AMENDMENT_PENDING', 'No REG-1205 approval yet carries a provenance_role_classification block; the post-merge provenance amendment (V12-B-1) is pending', 'REG-1205'));
    return findings;
  }
...
export function generate(ctx = loadContext()) {
  ...
  const chronology = {
    package: primary?.package ?? 'PACKAGE-12-1',
    inherited_release_tag: roles.inheritedTag ?? 'central-registration-volume-11-v1.0.0',
    sequence: [
  ...
  const roleClassification = {
    ...
    classification: {
      ...
      [roles.amendmentAuthoring ?? 'unknown']: 'V12_B_PROVENANCE_AMENDMENT_AUTHORING',
      [roles.amendmentMerge ?? 'unknown']: 'V12_B_PROVENANCE_AMENDMENT_MERGE'
    },
  ...
  const md = `# Volume 12 Package 1 Provenance-Integrity Report (NON-AUTHORITATIVE)
