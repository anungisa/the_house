// Control: Volume 9 provenance-integrity enforcement (deterministic, NON-AUTHORITATIVE).
//
// Proves, from the source-controlled Volume 9 corpus alone, that each package's
// recorded provenance is coherent and role-correct. It evaluates the authoritative
// provenance record(s) - the REG-905 approval(s) carrying a
...
function approvals(ctx) {
  return ctx.registers?.['REG-905']?.doc?.records ?? [];
}
...
function gateApproval(ctx) {
  return approvals(ctx).find((a) => /^GATE-V9-G[0-9]$/.test(a.artifact_id ?? '') && a.approval_state === 'ratified' && a.gate_disposition);
}
...
export function evaluateRecord(ctx, record) {
  ...
  const freezeArtifact = record.requires_freeze_artifact ?? rc.package ?? 'PACKAGE-9-1';
...
export function run(ctx) {
  ...
  if (records.length === 0) {
    // The authoritative provenance record (provenance_role_classification) is
    // recorded only by the post-merge provenance amendment (V9-B), because it must
    // bind the original-merge and amendment commits, which do not exist until after
    // the Package 1 merge. Before that amendment this control is not applicable and
    // reports an INFO signal; it does NOT pass judgement on absent evidence. Once a
    // record exists, evaluation is strictly fail-closed against the twelve
    // conditions below.
    findings.push(makeFinding(Severity.INFO, 'PROVENANCE_AMENDMENT_PENDING', 'No REG-905 approval yet carries a provenance_role_classification block; the post-merge provenance amendment (V9-B) is pending', 'REG-905'));
    return findings;
  }
...
export function generate(ctx = loadContext()) {
  ...
  const roleClassification = {
    ...
    classification: {
      ...
      [roles.amendmentAuthoring ?? 'unknown']: 'V9_B_PROVENANCE_AMENDMENT_AUTHORING',
      [roles.amendmentMerge ?? 'unknown']: 'V9_B_PROVENANCE_AMENDMENT_MERGE'
    },
